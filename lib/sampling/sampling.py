...
@sampling_utils.register_sampler
class ConditionalImageTauLeapingWithLabel():
    def __init__(self, cfg):
        self.cfg = cfg

    def sample(self, model, N, num_intermediates, conditioner, label):
        ...
        with torch.no_grad():
            ...
            for idx, t in tqdm(enumerate(ts[0:-1])):
                ...
                p0t = F.softmax(model(model_input, t * torch.ones((N,), device=device), label), dim=2) # (N, D, S)
            ...
            model_input = torch.concat((conditioner, x), dim=1)
            p_0gt = F.softmax(model(model_input, min_t * torch.ones((N,), device=device), label), dim=2) # (N, D, S)
...
@sampling_utils.register_sampler
class ConditionalImagePCTauLeapingWithLabel():
    def __init__(self, cfg):
        self.cfg = cfg
        ...
        with torch.no_grad():
            ...
            for idx, t in tqdm(enumerate(ts[0:-1])):
                ...
                def get_rates(in_x, in_t):
                    ...
                    p0t = F.softmax(model(model_input, in_t * torch.ones((N,), device=device), label), dim=2) # (N, D, S)
                    p0t = p0t[:, condition_dim:, :]
...
@sampling_utils.register_sampler
class ConditionalImageTauLeaping():
    def __init__(self, cfg):
        self.cfg = cfg
...
@sampling_utils.register_sampler
class ConditionalImagePCTauLeaping():
    def __init__(self, cfg):
        self.cfg = cfg
